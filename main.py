    ...
    with open(filepath, mode='r') as file:
        ...
        for row in csv_reader:
            ...
            if '.' in variant_feature:  # it is an attribute
                key = variant_feature
                variation_points[variant_feature] = VariationPoint(feature=get_feature_from_fms(vp_feature, fms), handler=vp_handler)
            elif not vp_feature in variation_points:
                key = vp_feature
                variation_points[vp_feature] = VariationPoint(feature=get_feature_from_fms(vp_feature, fms), handler=vp_handler)
            else:
                key = vp_feature
            if variant_value == '-':
                variant_value = None
...
def get_variant_value(fms: list[FeatureModel], variation_point: VariationPoint, configurations: list[Configuration], attributes: list[dict[str, str]]) -> str:
...
def get_variant_value_in_configuration(fms: list[FeatureModel], variation_point: VariationPoint, configuration: Configuration, attributes: dict[str, str]) -> str:
